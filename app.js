document.addEventListener('DOMContentLoaded', () => {
	...
	// Create a function that will listen up every time we click a square in grid. For every square, loop over the function
	for (let i = 0, len = squares.length; i < len; i++) {
		(function (index) {
			// add an onClick to each square in grid
			squares[i].onclick = function () {
				// if you click on a div and the div/sqr below your current square, or 7 sqrs down, has a className of 'taken', you're allowed to click on that div and can claim it as you taken. And you can go on top of it.
	...
	function checkBoard () {
		...
		// take the 4 values from each winningArrays and plug them into squares
		for (let j = 0; j < winningArrays.length; j++) {
			...
			if (
				...
				// if they do, player-one will be the winner
				result.innerHTML = 'Player One Wins!';
			} else if (
				// if they all have class name player-two
				square1.classList.contains('player-two') &&
				square2.classList.contains('player-two') &&
				square3.classList.contains('player-two') &&
				square4.classList.contains('player-two')
			) {
				// if the ans is true, player-two will be the winner
